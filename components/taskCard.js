    ...
    <div className={styles.container} id={styles.taskContainer} style={style}>
      ...
      <Row style={{ margin: '0 10px 0 2px', justifyContent: 'space-between' }}>
        ...
        <div className={styles.notititle}>
          {Math.floor(sumTime)} h  {' '}
          {((Math.round(((sumTime) % 1) * 60) !== 0) && Math.round(((sumTime) % 1) * 60) < 10)
            ? `0${Math.round(((sumTime) % 1) * 60)}`
            : Math.round(((sumTime) % 1) * 60)}{' '}
          sur {Math.floor(props.task.budget)}h{' '}
          {((Math.round(((props.task.budget) % 1) * 60) !== 0) && Math.round(((props.task.budget) % 1) * 60) < 10)
            ? `0${Math.round(((props.task.budget) % 1) * 60)}`
            : Math.round(((props.task.budget) % 1) * 60)}
        </div>
      </Row>
